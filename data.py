...
st.sidebar.markdown("2023年ヴィクトリアマイル　データ")
sample2=pd.read_csv("Victoria Mile 2023.csv")
sample2=sample2.to_csv(index=False) 
b64 = base64.b64encode(sample2.encode('utf-8-sig')).decode()
href = f'<a href="data:application/octet-stream;base64,{b64}" download="Victoria Mile 2023.csv">Victoria Mile 2023</a>'
st.sidebar.markdown(f"{href}", unsafe_allow_html=True)
...
if a=="全レース":
  ...
  y=st.selectbox("レース選択",("フェブラリーステークス","高松宮記念","大阪杯","桜花賞","皐月賞","天皇賞（春）","NHKマイルカップ","ヴィクトリアマイル","エリザベス女王杯","マイルチャンピオンシップ","ジャパンカップ","チャンピオンズカップ","阪神ジュベナイルフィリーズ","朝日杯フューチュリティステークス","有馬記念"))
  if y=="エリザベス女王杯":
