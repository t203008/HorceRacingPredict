...
st.sidebar.markdown("2023年安田記念　データ")
sample2=pd.read_csv("Yasuda Kinen 2023.csv")
sample2=sample2.to_csv(index=False) 
b64 = base64.b64encode(sample2.encode('utf-8-sig')).decode()
href = f'<a href="data:application/octet-stream;base64,{b64}" download="Yasuda Kinen 2023.csv">Yasuda Kinen 2023</a>'
st.sidebar.markdown(f"{href}", unsafe_allow_html=True)
...
if a=="全レース":
  ...
  y=st.selectbox("レース選択",("フェブラリーステークス","高松宮記念","大阪杯","桜花賞","皐月賞","天皇賞（春）","NHKマイルカップ","ヴィクトリアマイル","優駿牝馬（オークス）","東京優駿（日本ダービー）","安田記念","エリザベス女王杯","マイルチャンピオンシップ","ジャパンカップ","チャンピオンズカップ","阪神ジュベナイルフィリーズ","朝日杯フューチュリティステークス","有馬記念"))
  if y=="エリザベス女王杯":
...
if pred is not None:
  ...
  if waku=="入れる":
    ...
    logistic1 = smf.glm(formula = "Win ~ 1+Age+Mare+Stallion+P_rank+P_popular*P_popular+P_popular+Jockey_change+Change_from_P_Grass+Change_from_P_Dirt+Change_from_P_Hurdle+P_class_Class+Weight_P_Weight+Distance_P_distance+Week_distance+P_overseas",
                   data = Z ,
                   family = sm.families.Binomial()).fit()
