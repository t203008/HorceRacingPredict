...
if a=="全レース":
  ...
  y=st.selectbox("レース選択",("フェブラリーステークス","高松宮記念","大阪杯","桜花賞","皐月賞","天皇賞（春）","NHKマイルカップ","ヴィクトリアマイル","優駿牝馬（オークス）","東京優駿（日本ダービー）","安田記念","宝塚記念","スプリンターズステークス","秋華賞","天皇賞（秋）","エリザベス女王杯","マイルチャンピオンシップ","ジャパンカップ","チャンピオンズカップ","中日新聞杯","カペラステークス","阪神ジュベナイルフィリーズ","ターコイズステークス","朝日杯フューチュリティステークス","阪神カップ","有馬記念"))
  if y=="エリザベス女王杯":
...
if pred is not None:
  ...
  if waku=="入れる":
    ...
    logistic1 = smf.glm(formula = "Win ~ 1+Age+Age*Age+Frame_one+Frame_two+Frame_three+Frame_four+Frame_five+Frame_six+Frame_seven+Frame_eight+Mare+Stallion+P_rank+P_popular+Jockey_change+Change_from_P_Grass+Change_from_P_Dirt+Change_from_P_Hurdle+P_class_Class+Weight_P_Weight+Distance_P_distance+Week_distance+P_overseas+P_rank*P_popular*P_class_Class",
                   data = Z ,
                   family = sm.families.Binomial()).fit()
    logistic2 = smf.glm(formula = "Quinella ~ 1+Age+Age*Age+Frame_one+Frame_two+Frame_three+Frame_four+Frame_five+Frame_six+Frame_seven+Frame_eight+Mare+Stallion+P_rank+P_popular+Jockey_change+Change_from_P_Grass+Change_from_P_Dirt+Change_from_P_Hurdle+P_class_Class+Weight_P_Weight+Distance_P_distance+Week_distance+P_overseas+P_rank*P_popular*P_class_Class",
                   data = Z ,
                   family = sm.families.Binomial()).fit()
    logistic3 = smf.glm(formula = "Show ~ 1+Age+Age*Age+Frame_one+Frame_two+Frame_three+Frame_four+Frame_five+Frame_six+Frame_seven+Frame_eight+Mare+Stallion+P_rank+P_popular+Jockey_change+Change_from_P_Grass+Change_from_P_Dirt+Change_from_P_Hurdle+P_class_Class+Weight_P_Weight+Distance_P_distance+Week_distance+P_overseas+P_rank*P_popular*P_class_Class",
                   data = Z ,
                   family = sm.families.Binomial()).fit()
  elif waku=="入れない":
    ...
    logistic1 = smf.glm(formula = "Win ~ 1+Age+Age*Age+Mare+Stallion+P_rank+P_popular*P_popular+Jockey_change+Change_from_P_Grass+Change_from_P_Dirt+Change_from_P_Hurdle+P_class_Class+Weight_P_Weight+Distance_P_distance+Week_distance+P_overseas",
                   data = Z ,
                   family = sm.families.Binomial()).fit()
    logistic2 = smf.glm(formula = "Quinella ~ 1+Age+Age*Age+Mare+Stallion+P_rank+P_popular*P_popular+P_popular+Jockey_change+Change_from_P_Grass+Change_from_P_Dirt+Change_from_P_Hurdle+P_class_Class+Weight_P_Weight+Distance_P_distance+Week_distance+P_overseas",
                   data = Z ,
                   family = sm.families.Binomial()).fit()
    logistic3 = smf.glm(formula = "Show ~ 1+Age+Age*Age+Mare+Stallion+P_rank+P_popular*P_popular+P_popular+Jockey_change+Change_from_P_Grass+Change_from_P_Dirt+Change_from_P_Hurdle+P_class_Class+Weight_P_Weight+Distance_P_distance+Week_distance+P_overseas",
                   data=Z,
                   family = sm.families.Binomial()).fit() 
